...
menu = st.sidebar.selectbox("메뉴", ["문제 추가하기", "문제 조회 및 PDF 생성"])
...
# ------------------------------------------
    ...
    if filtered_questions:
        ...
        if st.button("PDF 시험지 생성하기", type="primary"):
            ...
            with st.spinner('깔끔한 2단 PDF를 조판하고 있습니다...'):
                # 이전 단계에서 수정한 create_exam_pdf 함수에 exam_title 인자를 넘겨줍니다.
                result = create_exam_pdf(final_list, filename, answer_mode, exam_title=selected_exam+' '+selected_subject)
            ...
            if result == "성공":
                ...
                with open(filename, "rb") as pdf_file:
                    st.download_button(
                        label="📥 만들어진 시험지 다운로드",
                        data=pdf_file,
                        # 다운로드 파일 이름에도 시험 이름이 포함되도록 수정
                        file_name=f"{selected_exam}_{selected_subject}_모의고사.pdf",
